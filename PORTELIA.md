...
| `tailwind.config.js` | Incluye `dashboard/portelia/**/*.js` para compilar las utilities de `clasesAsesor.js` también en un build limpio, no sólo en la caché HMR. |
| `app/javascript/dashboard/featureFlags.js` | Espejo del flag: `FEATURE_FLAGS.PORTELIA_UI`. |
| `app/javascript/dashboard/components-next/dialog/Dialog.vue` | Prop opt-in `dialogClass`, vacía por defecto. `portelia/components/DialogoAsesor.vue` adapta el modal al teléfono con utilities. El Teleport no hereda clases del caller; este enchufe evita duplicar Dialog. Sin la prop, upstream no cambia. |
...
| `app/javascript/dashboard/routes/dashboard/Dashboard.vue` | Con el flag, en el celular monta `BarraInferiorAsesor` en vez de `MobileSidebarLauncher` y le deja lugar abajo al `<main>` (`useBarraInferior`). Con `portelia_ui` agrega safe areas y mínimos táctiles móviles a los controles stock de Bandeja/Personas/hilo. Las utilities viven en `portelia/clasesAsesor.js`; ajustan el alto del header, distribuyen búsqueda/filtros de Personas y permiten el wrap del compositor para los nuevos tamaños. También dejan pasar taps por la franja invisible del panel de contacto cerrado. El flag apagado conserva las clases previas. |
