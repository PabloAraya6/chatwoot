...
const aPropiedad = () => {
  ...
  NUMERICOS.forEach(campo => {
    if (String(campos[campo]).trim() !== '')
      propiedad[campo] = Number(campos[campo]);
  });
...
const desdePropiedad = propiedad => {
  ...
  NUMERICOS.forEach(campo => {
    campos[campo] =
      propiedad[campo] === undefined ? '' : String(propiedad[campo]);
  });
  TEXTOS.forEach(campo => {
  ...
  <section class="flex w-full h-full overflow-hidden bg-n-surface-1">
    <div class="flex flex-col w-full h-full">
      ...
      <main class="flex-1 px-6 pb-6 overflow-y-auto">
        ...
        >
          <fieldset
            class="grid grid-cols-1 gap-4 p-0 m-0 border-0 sm:grid-cols-2"
          >
            ...
            <label class="flex flex-col gap-1">
              ...
              <Select
                v-model="campos.operacion"
                :options="opciones.operacion.value"
                class="!w-full [&>select]:w-full"
              />
            </label>
            <label v-if="!esNueva" class="flex flex-col gap-1">
              ...
              <Select
                ...
                :options="opciones.estado.value"
                class="!w-full [&>select]:w-full"
              />
            </label>
            <Input
              v-model="campos.direccion"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.DIRECCION')"
            />
            <Input
              v-model="campos.zona"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.ZONA')"
            />
            <Input
              v-model="campos.ciudad"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.CIUDAD')"
            />
          </fieldset>

          <fieldset
            class="grid grid-cols-2 gap-4 p-0 m-0 border-0 sm:grid-cols-3"
          >
            <Input
            ...
            <label class="flex flex-col gap-1">
              ...
              <Select
                v-model="campos.moneda"
                :options="opciones.moneda.value"
                class="!w-full [&>select]:w-full"
              />
            </label>
            <Input
          ...
          <fieldset class="flex flex-col gap-4 p-0 m-0 border-0">
            <Input
              v-model="campos.condicion"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.CONDICION')"
              :placeholder="
                t('PORTELIA.PROPIEDADES.FORMULARIO.CONDICION_EJEMPLO')
              "
            />
            <TextArea
              v-model="campos.descripcion"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.DESCRIPCION')"
              :max-length="4000"
              auto-height
            />
            <Input
              v-model="campos.amenities"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.AMENITIES')"
              :message="t('PORTELIA.PROPIEDADES.FORMULARIO.AMENITIES_AYUDA')"
            />
            <TextArea
              v-model="campos.fotos"
              :label="t('PORTELIA.PROPIEDADES.FORMULARIO.FOTOS')"
              :max-length="8000"
              auto-height
            />
