...
const si = valor => t(valor ? 'PORTELIA.PROPIEDADES.FICHA.SI' : 'PORTELIA.PROPIEDADES.FICHA.NO');
...
const datos = computed(() => {
  ...
  return [
    ['PRECIO', precioDe(p, t)],
    ['EXPENSAS', p.expensas !== undefined ? p.expensas.toLocaleString('es-AR') : null],
    ['TIPO', t(`PORTELIA.PROPIEDADES.TIPO.${p.tipo}`)],
    ['OPERACION', t(`PORTELIA.PROPIEDADES.OPERACION.${p.operacion}`)],
    ['SUPERFICIE_CUBIERTA', p.superficieCubiertaM2 ? m2(p.superficieCubiertaM2) : null],
...
const cargar = async () => {
  cargando.value = true;
  try {
    propiedad.value = (await miApi.get(`propiedades/${props.propiedadId}`)).data;
  } catch {
        ...
        >
          ...
          <div class="flex flex-col gap-2">
            <div class="flex flex-wrap items-center gap-1.5">
              ...
              />
              <Label
                :label="t(`PORTELIA.PROPIEDADES.OPERACION.${propiedad.operacion}`)"
                compact
              />
              <Label
          ...
          >
            <div v-for="[clave, valor] in datos" :key="clave" class="flex flex-col gap-0.5">
              <dt class="text-xs font-medium uppercase tracking-wide text-n-slate-11">
          ...
          <section v-if="propiedad.amenities.length" class="flex flex-col gap-2">
            <h2 class="mb-0 text-base font-medium text-n-slate-12">
